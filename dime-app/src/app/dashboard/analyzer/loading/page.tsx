...
export default function AnalyzerLoadingPage() {
  ...
  useEffect(() => {
    ...
    const progressTimer = window.setInterval(() => {
      setProgress((currentProgress) => {
        const nextProgress = Math.min(currentProgress + 4, 92);
  ...
  const handleRetry = () => {
    if (ideaId) {
      router.replace(`/dashboard/analyzer/loading?ideaId=${ideaId}`);
    }
  };
  ...
  return (
    <div className="w-full max-w-2xl mx-auto min-h-[calc(100vh-140px)] flex flex-col items-center justify-center -mt-10 animate-fade-in">
      <div className="mb-8 rounded-xl border border-slate-200 bg-white p-4 text-[#ea580c] shadow-sm">
        <LineChart className="h-8 w-8" />
      </div>

      <h2 className="text-3xl font-extrabold text-slate-900 mb-3 text-center">
        Running idea analysis...
      </h2>
      <p className="text-slate-500 font-medium text-center max-w-md mx-auto mb-12">
            ...
            <div key={step.id} className="flex gap-4">
              <div className="flex-shrink-0 mt-0.5">
                {isCompleted ? (
                  ...
                  <div className="w-6 h-6 rounded-full border-2 border-slate-200 flex items-center justify-center">
                    <Loader2 className="w-3 h-3 text-slate-300" />
                  </div>
                )}
              </div>
              ...
              >
                <div className="flex justify-between items-center mb-1">
                  <h4
                    className={`font-bold text-sm ${isCurrent ? "text-slate-900" : "text-slate-700"}`}
                  >
                    {step.title}
                  </h4>
                  {isCurrent && (
      ...
      <div className="mt-12 flex items-center justify-center text-sm font-medium text-slate-400">
